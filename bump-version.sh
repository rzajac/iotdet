...
if [ -f VER ]; then
    ...
    read
    echo -e "$PUSHING_MSG"
    git add CHANGELOG.md VER
    git commit -m "Bump version to ${INPUT_STRING}."
    git tag -a -m "Tag version ${INPUT_STRING}." "v$INPUT_STRING"
    git push origin --tags
else
    echo -e "${WARNING_FLAG} Could not find a VER file."
    ...
    if [ "$RESPONSE" = "y" ]; then
        ...
        read
        echo -e "$PUSHING_MSG"
        git add VER CHANGELOG.md
        git commit -m "Add VER and CHANGELOG.md files, Bump version to v0.1.0."
